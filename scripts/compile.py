...
def load_chapters():
    ...
    for f in sorted(os.listdir(CH)):
        ...
        out.append({"num": num, "title": meta.get("title", ""), "source": meta.get("source", ""),
                    "paras": paras, "file": f})
    return out
...
def build_index(chaps):
    lines = [f"# {TITLE} — Index", "", f"{len(chaps)} chapters", ""]
    for c in chaps:
        lines.append(f"- [{c['num']}. {c['title'] or ''}](chapters/{c['file']})")
...
def build_epub(chaps, path):
    ...
    for c in chaps:
        ...
        navs.append(f'<li><a href="{cid}.xhtml">{label}</a></li>')
